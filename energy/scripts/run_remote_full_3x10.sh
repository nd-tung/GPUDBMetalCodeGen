...
RUN_ID=${RUN_ID:-tpch-sf10-shelly-full-run12}
...
# There are 17 TPCH CLI operators. The three aggregate operators each have two
# GPU reductions, group-by has two GPU aggregation strategies, and scan-sum has
# three CPU plus three GPU kernels:
#   13 regular operators * 2 backends
#   + 3 aggregates * (1 CPU + 2 GPU)
#   + 1 group-by * (1 CPU + 2 GPU)
#   + 6 scan-sum kernels
#   = 44 cases/trial, 132 cases over three trials.
EXPECTED_CASES=132
...
aggregate_forward=(aggregate-sum aggregate-minmax aggregate-stats)
aggregate_reverse=(aggregate-stats aggregate-sum aggregate-minmax)
aggregate_rotated=(aggregate-minmax aggregate-stats aggregate-sum)
...
expected_config=$(
  ...
  print -- "trials=3"
  print -- "cases_per_trial=44"
  print -- "expected_cases=$EXPECTED_CASES"
  print -- "operators=scan-copy,filter-count,filter-bitmap,filter-materialize,filter-project,hash-build,hash-probe-count,hash-probe-materialize,aggregate-sum,aggregate-minmax,aggregate-stats,groupby-part-count,q6-revenue,q6-revenue-unfused,q1-groupby,q14-join,orders-topk"
  print -- "techniques=q6-fused-vs-unfused,aggregate-simdgroup-vs-threadgroup-tree,groupby-global-atomic-vs-bounded-threadgroup,scan-cpu-scalar-parallel-simd,scan-gpu-baseline-multi-item-simdgroup"
  print -- "regular_pair_order=trial1-alternating-gpu-first,trial2-flipped,trial3-rotated-alternating-gpu-first"
)
if [[ -e "$CONFIG" ]]; then
...
if [[ ! -x "$ROOT/build/joule-measure" ]]; then
  print -u2 -- "missing executable: $ROOT/build/joule-measure"
  exit 1
fi
if [[ ! -x "$ROOT/build/joule-tpch-benchmark" ]]; then
  print -u2 -- "missing executable: $ROOT/build/joule-tpch-benchmark"
  exit 1
fi
if [[ ! -x "$ROOT/build/joule-benchmark" ]]; then
  print -u2 -- "missing executable: $ROOT/build/joule-benchmark"
  exit 1
fi
...
for colbin in lineitem.colbin part.colbin orders.colbin; do
...
artifact_identity() {
  ...
  for dataset_file in \
    ...
    "$data_directory/part.colbin" \
    "$data_directory/orders.colbin"; do
...
if [[ ! -e "$ENVIRONMENT" ]]; then
  {
    ...
    print -- "metal_compiler:"
    /usr/bin/xcrun -sdk macosx metal --version 2>/dev/null || true
  } >"$ENVIRONMENT"
fi
...
run_regular_pair() {
  ...
  # Alternate both directions within every trial. Trial 2 flips the parity;
  # trial 3 restores it after rotating the operator sequence.
  if (( (trial == 1 || trial == 3) && pair_index % 2 == 1 ||
        trial == 2 && pair_index % 2 == 0 )); then
...
run_regular_sequence() {
  local trial=$1
  shift
  local operator
  for operator in "$@"; do
    (( regular_pair_index += 1 ))
    run_regular_pair "$trial" "$operator" "$regular_pair_index"
  done
}

run_aggregate_variants() {
  local trial=$1
  local operator=$2

  # A three-way Latin rotation puts CPU, SIMD-group, and thread-group-tree in
  # each within-operator position exactly once across the three trials.
  case "$trial" in
    1)
      run_case "$trial" "gpu-$operator-simdgroup" "$operator" gpu \
        --gpu-aggregate-reduction simdgroup
      run_case "$trial" "cpu-$operator" "$operator" cpu
      run_case "$trial" "gpu-$operator-threadgroup-tree" "$operator" gpu \
...
run_aggregate_sequence() {
  local trial=$1
  shift
  local operator
  for operator in "$@"; do
    run_aggregate_variants "$trial" "$operator"
  done
}

run_groupby_variants() {
  local trial=$1

  # The same Latin rotation balances the three group-by implementations.
  case "$trial" in
    1)
      run_case "$trial" gpu-groupby-part-count-global-atomic \
        groupby-part-count gpu --gpu-groupby-strategy global-atomic
      run_case "$trial" cpu-groupby-part-count groupby-part-count cpu
      run_case "$trial" gpu-groupby-part-count-bounded-threadgroup \
        groupby-part-count gpu --gpu-groupby-strategy bounded-threadgroup
      ;;
    2)
      run_case "$trial" cpu-groupby-part-count groupby-part-count cpu
      run_case "$trial" gpu-groupby-part-count-bounded-threadgroup \
...
run_scan_variants() {
  ...
  case "$trial" in
    1)
      run_scan_case "$trial" gpu-scan-baseline gpu baseline
      run_scan_case "$trial" cpu-scan-scalar cpu scalar
      run_scan_case "$trial" gpu-scan-multi-item gpu multi-item
      run_scan_case "$trial" cpu-scan-parallel cpu parallel
      run_scan_case "$trial" gpu-scan-simdgroup gpu simdgroup
      run_scan_case "$trial" cpu-scan-simd cpu simd
      ;;
    2)
      run_scan_case "$trial" cpu-scan-simd cpu simd
      run_scan_case "$trial" gpu-scan-simdgroup gpu simdgroup
...
print -- \
  "cases=$EXPECTED_CASES trials=3 cases_per_trial=44 duration_ms=$DURATION_MS baseline_ms=$BASELINE_MS cooldown_ms=$COOLDOWN_MS nominal_total_ms=$NOMINAL_TOTAL_MS powermetrics_sample_rate_ms=$SAMPLE_RATE_MS shelly_sample_rate_ms=$SHELLY_SAMPLE_RATE_MS shelly_timeout_ms=$SHELLY_TIMEOUT_MS shelly_attempts=$SHELLY_ATTEMPTS cooperative_timeout_ms=$COOPERATIVE_TIMEOUT_MS warmups=$WARMUP_ITERATIONS group_cardinality=$GROUP_CARDINALITY scan_rows=$SCAN_ROWS scan_pattern=signed scan_batch_size=1 threadgroup_width=$THREADGROUP_WIDTH artifact_lock=$ARTIFACT_LOCK" |
  tee -a "$LOG"
print -- \
...
print -- \
  ...
  tee -a "$LOG"
print -- \
  "coverage=17-tpch-operators-cpu-gpu,6-scan-sum-kernels variants=q6-fused-unfused,aggregate-simdgroup-threadgroup-tree,groupby-global-atomic-bounded-threadgroup" |
  tee -a "$LOG"

for trial in 1 2 3; do
  print -- "$(date -u +%FT%TZ) TRIAL $trial" | tee -a "$LOG"
  regular_pair_index=0
  case "$trial" in
    1)
      run_scan_variants "$trial"
      run_regular_sequence "$trial" "${regular_forward[@]}"
      run_aggregate_sequence "$trial" "${aggregate_forward[@]}"
      run_groupby_variants "$trial"
      ;;
    2)
      run_groupby_variants "$trial"
      run_aggregate_sequence "$trial" "${aggregate_reverse[@]}"
      run_scan_variants "$trial"
      run_regular_sequence "$trial" "${regular_reverse[@]}"
      ;;
    3)
      # Place the technique blocks in the middle of the rotated regular list
      # so they are not consistently measured at either trial boundary.
      run_regular_sequence "$trial" "${(@)regular_rotated[1,7]}"
      run_aggregate_sequence "$trial" "${aggregate_rotated[@]}"
      run_groupby_variants "$trial"
      run_regular_sequence "$trial" "${(@)regular_rotated[8,13]}"
      run_scan_variants "$trial"
      ;;
  esac
done
